...
if(!empty($_POST)) { # Création, mise à jour, suppression ou aperçu
	...
	$title_page = $ADM_articleedit_title;
	...
	$title_page = $ADM_newarticle_title;
}
...
<form action="article.php" method="post" id="change-art-content">
	<fieldset>
		...
		<p class="field">
			...
			<?php plxUtils::printInput('title',htmlspecialchars($title,ENT_QUOTES,PLX_CHARSET),'text','50-255'); ?> <?php echo $ADM_postas; ?> <strong><?php echo $author; ?></strong>
		</p>
		<p class="field">
		...
		<p class="field">
			...
			<input type="submit" name="delete" value="<?php echo $ADM_newarticle_delete; ?>" onclick="Check=confirm('Отменить запись ?');if(Check==false) return false;"/>
		</p>
	</fieldset>
</form>
