...
if($plxAdmin->plxGlob_arts->count AND $plxAdmin->plxRecord_arts->size) { # On a des articles
	while ($plxAdmin->plxRecord_arts->loop()) { # Pour chaque article
		...
		if($publi) # Si l'article est publié
			echo '<a href="'.PLX_ROOT.'?article'.intval($plxAdmin->plxRecord_arts->f('numero')).'/'.$plxAdmin->plxRecord_arts->f('url').'" title="Просмотреть новость">'.$ADM_newarticle_preview.'</a> - ';
		echo '<a href="article.php?a='.$plxAdmin->plxRecord_arts->f('numero').'" title="Изменить новость">'.$ADM_gal_editgallery.'</a> - ';
echo '<a href="news.php?del='.$plxAdmin->plxRecord_arts->f('numero').'&amp;hash='.$_SESSION['hash'].'" onclick="Check=confirm(\''.$ADM_check_deletion.'\');if(Check==false) return false;" title="Удаление новости">'.$ADM_newarticle_delete.'</a></td></tr>';
