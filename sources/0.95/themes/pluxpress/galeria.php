			...
			If((!preg_match("/.tb/i",$file)) && (in_array(file_ext($file),$show_files))) {
				...
				If(!preg_match("/$ignore_word/i",$file)) {
					...
					If($auto_thumb) {
						...
						plxUtils::makeThumb($full_server.$file, $full_server.$file.'.tb',$thumb_width,$thumb_height,$jpg_quality,$thumb_offset,$plxShow->plxMotor->thumbtype);	
						}
					}
...
$opacity = 'filter:alpha(opacity='.($kf*100).'); -moz-opacity: '.$kf.'; -khtml-opacity: '.$kf.'; opacity: '.$kf.';';
					...
					$rel = 'rel="lytebox"';
					if ($plxShow->plxMotor->images_sets==1) $rel = 'rel="lytebox[images]"'; 
					...
					if ($plxShow->plxMotor->imgorderby==1) { // sort by date
						$images[$filedate] .= "<div class=\"framer ".$file0."\">".$fresh_label."<a class=\"galimage\" style=\"background:url(".$full_server.$thumb.") no-repeat center; width:".$thumb_width."px; height:".$thumb_height."px;".$opacity."\"  href=\"".$files_path.$album.$file_name.$file_ext."\" ".$rel." title=\"".$file_description."\"></a>".$filename."</div>\n";
					}
					$k++;						
