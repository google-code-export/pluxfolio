...
class plxFeed extends plxMotor {
	...
	function getAtomCommentaires() {
		...
		if($this->cible) { # Commentaires d'un article
			...
			$link = $this->aConf['racine'].'?article'.$artId.'/'.$this->plxRecord_arts->f('url');
			$link_feed = $this->aConf['racine'].'feed.php?atom/commentaires/article'.$artId;
		} else { # Commentaires globaux
			$title = $this->aConf['title'].' - Commentaires';
			$link = $this->aConf['racine'];
			$link_feed = $this->aConf['racine'].'feed.php?atom/commentaires';
		}
		...
		if($this->plxRecord_coms) {
			while($this->plxRecord_coms->loop()) {
				...
				if($this->cible) { # Commentaires d'un article
					...
					$title_com .= plxUtils::dateIsoToHum($this->plxRecord_coms->f('date')).', ';
					$title_com .= plxUtils::heureIsoToHum($this->plxRecord_coms->f('date'));
					$link_com = $this->aConf['racine'].'?article'.$artId.'/'.$this->plxRecord_arts->f('url').'/#c'.$this->plxRecord_coms->f('numero');
				} else { # Commentaires globaux
					$title_com = $this->plxRecord_coms->f('author').' le ';
					$title_com .= plxUtils::dateIsoToHum($this->plxRecord_coms->f('date')).', ';
					$title_com .= plxUtils::heureIsoToHum($this->plxRecord_coms->f('date'));
					$link_com = $this->aConf['racine'].'?article'.$artId.'/#c'.$this->plxRecord_coms->f('numero');
				}
				# On check la date de publication
				...
				# On affiche le flux dans un buffer
				$entry .= '<entry>'."\n";
				$entry .= "\t".'<title type="html">'.htmlspecialchars($title_com,ENT_QUOTES,PLX_CHARSET).'</title> '."\n";
				$entry .= "\t".'<link href="'.$link_com.'"/>'."\n";
				$entry .= "\t".'<id>urn:md5:'.md5($link_com).'</id>'."\n";
				$entry .= "\t".'<updated>'.$this->plxRecord_coms->f('date').'</updated>'."\n";
				$entry .= "\t".'<author><name>'.htmlspecialchars($this->plxRecord_coms->f('author'),ENT_QUOTES,PLX_CHARSET).'</name></author>'."\n";
				$entry .= "\t".'<content type="html">'.htmlspecialchars($this->plxRecord_coms->f('content'),ENT_QUOTES,PLX_CHARSET).'</content>'."\n";
				$entry .= '</entry>'."\n";
			}
		}
		...
		echo '<?xml version="1.0" encoding="'.PLX_CHARSET.'" ?>'."\n";
		echo '<feed xmlns="http://www.w3.org/2005/Atom" xmlns:dc="http://purl.org/dc/elements/1.1/">'."\n";
		echo '<title type="html">'.htmlspecialchars($title,ENT_QUOTES,PLX_CHARSET).'</title>'."\n";
		echo '<subtitle type="html">'.htmlspecialchars($this->aConf['description'],ENT_QUOTES,PLX_CHARSET).'</subtitle>'."\n";
	...
	function getRssCommentaires() {
		...
		if($this->plxRecord_coms) {
			while($this->plxRecord_coms->loop()) {
				...
				$entry .= "\t".'<title>'.htmlspecialchars($title_com,ENT_QUOTES,PLX_CHARSET).'</title> '."\n";
				$entry .= "\t".'<link>'.$link_com.'</link>'."\n";
				$entry .= "\t".'<dc:date>'.$this->plxRecord_coms->f('date').'</dc:date>'."\n";
				$entry .= "\t".'<dc:creator>'.htmlspecialchars($this->plxRecord_coms->f('author'),ENT_QUOTES,PLX_CHARSET).'</dc:creator>'."\n";
				$entry .= "\t".'<description>'.htmlspecialchars($this->plxRecord_coms->f('content'),ENT_QUOTES,PLX_CHARSET).'</description>'."\n";
				$entry .= '</item>'."\n";
			}
		}
		...
		echo "\t".'<title>'.htmlspecialchars($title,ENT_QUOTES,PLX_CHARSET).'</title>'."\n";
		echo "\t".'<link>'.$link.'</link>'."\n";
		echo "\t".'<description>'.htmlspecialchars($this->aConf['description'],ENT_QUOTES,PLX_CHARSET).'</description>'."\n";
		echo "\t".'<lastBuildDate>'.$last_updated.'</lastBuildDate>'."\n";
		echo "\t".'<generator>Pluxml '.$this->version.'</generator>'."\n";
