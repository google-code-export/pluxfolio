...
if(isset($_GET['del']) AND !empty($_GET['hash']) AND $_GET['hash'] == $_SESSION['hash'] AND isset($aDoc[ $_GET['del'] ]) AND file_exists($dirDoc.$aDoc[ $_GET['del'] ])) {
	...
	if(!@unlink($dirDoc.$aDoc[ $_GET['del'] ])) # Erreur de suppression
		$msg = 'Impossible de supprimer le document (probl&egrave;me d\'&eacute;criture dans le dossier '.$dirDoc.')';
	else # Ok
		$msg = 'Документ удалён в '.date('H:i:s');
	header('Location: documents.php?msg='.urlencode($msg));
	exit;
}
...
if(!empty($_FILES) AND !empty($_FILES['doc']['name'])) {
	...
	if(!@move_uploaded_file($_FILES['doc']['tmp_name'],$upFile)) # Erreur de copie
		$msg = 'Impossible d\'envoyer le document (probl&egrave;me d\'&eacute;criture dans le dossier '.$dirDoc.')';
	else # Ok
		$msg = 'Документ загружен в '.date('H:i:s');
...
<?php include("top.php"); ?>
	...
	<h2>Закачка документов на сайт</h2>
	...
	<form enctype="multipart/form-data" action="documents.php" method="post">
		<fieldset class="withlabel">
			<legend>Формочка закачки</legend>
			<p><input name="doc" type="file" />
			<input type="submit" name="upload" value="к0чай!" /></p>
		</fieldset>
	</form>
	<h3 class="subh">Закачанные файлы</h3>
	<table class="list-articles">
	<thead>
		<tr>
			<th width="65%">Имя файла</th>
			<th width="15%">Что с ним сделать?</th>
		</tr>
	</thead>
	<tbody>
...
<?php } else { # On a des documents
	...
	for($i=0; $i < $nb; $i++) { ?>
		<tr class="line-<?php echo $i%2; ?>">
			...
			<td class="tc1">&nbsp;
				<a href="documents.php?del=<?php echo $i.'&amp;hash='.$_SESSION['hash'] ?>" title="удалить файл с сайта" onClick="Check=confirm('Удалить файл ?');if(Check==false) return false;">
				<img src="img/delete.png" alt="Удалятор" /></a>
			</td>
        </tr>
