		...
		<?php
			if(!$aImg) { # Aucune image
				...
				for($i=0; $i < $nb; $i++) 
					...
					echo '<a href="?folder='.$folder.'&del='.$i.'&amp;hash='.$_SESSION['hash'].'" title="удалить картинку" onclick="Check=confirm(\''.$ADM_check_deletion.'\');if(Check==false) return false;"><img src="img/delete.png" alt="удаление" /></a></p>';
					echo '</div>'."\n";
					}
			}
		?>
